  ...
  return (
    <div className="rounded-2xl border p-6">
      ...
      <div className="mt-5 flex flex-wrap gap-2">
        ...
        <Link className="rounded-xl border px-3 py-2 text-sm hover:bg-muted" href="/ar/kemik">
          اذهب إلى كيميك
        </Link>
      </div>
    </div>
...
export default async function ArabicDeriHome() {
  const stories: StoryCard[] = await prisma.story.findMany({
    where: { language: "ar", author: "deri" },
  ...
  return (
    <main dir="rtl" className="mx-auto w-full max-w-5xl px-4 py-10">
      <header className="mb-8">
        <h1 className="text-3xl font-semibold tracking-tight">ديري</h1>
        <p className="mt-2 text-sm text-muted-foreground">قصص ديري بالعربية</p>
