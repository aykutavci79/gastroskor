...
function InsightList({
  ...
  if (items.length === 0) {
    return (
      <section className="rounded-xl border border-border/70 bg-surface-card p-4">
        <h3 className={`text-sm font-semibold ${accent}`}>{title}</h3>
        <p className="mt-2 text-sm text-content-muted">Bu bolum icin yeterli son 3 ay yorumu bulunamadi.</p>
      </section>
    );
  }
  ...
  return (
    <section className="rounded-xl border border-border/70 bg-surface-card p-4">
      ...
      <ul className="mt-3 space-y-3">
        {items.map((item, idx) => (
          <li key={idx} className="rounded-lg border border-border bg-surface-input p-3">
            <p className="text-xs uppercase tracking-wider text-content-muted">{item.category}</p>
            <p className="mt-1 text-sm text-content">{item.summary}</p>
            ...
            {item.evidence_quotes.length > 0 ? (
              <div className="mt-2 space-y-1">
                {item.evidence_quotes.map((quote, qIdx) => (
                  <p key={qIdx} className="text-xs italic text-content-muted">
                    &quot;{quote}&quot;
                  </p>
                ))}
              </div>
...
export function CompetitorAiReportView({ report, onClose }: Props) {
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/70 p-4 sm:items-center">
      <div className="max-h-[90vh] w-full max-w-3xl overflow-y-auto rounded-2xl border border-border bg-surface-input p-6 shadow-2xl">
        ...
        <div className="mt-4 rounded-xl border border-violet-500/30 bg-violet-500/10 p-4">
          ...
          <p className="mt-2 text-sm leading-relaxed text-content">{report.comparison_summary}</p>
        ...
        {report.warnings.length > 0 ? (
          <ul className="mt-3 space-y-1 rounded-lg border border-amber-500/30 bg-amber-500/10 p-3 text-xs text-brand-gold">
        ...
        <div className="mt-4 grid gap-4 lg:grid-cols-2">
          ...
          />
          <InsightList title="Sizin guclu yanlar" items={report.your_strengths} accent="text-success" />
        </div>
