...
export function resolveRatingBandVisual(rating: number | null | undefined): RatingBandVisual | null {
  ...
  if (rating >= 4.5) {
    return {
      band: 'elite',
      stripe: GastroColors.gold,
      accent: GastroColors.gold,
      softBackground: hexToRgba(GastroColors.gold, 0.18),
    };
  }
  if (rating >= 4.0) {
    return {
      band: 'good',
      stripe: GastroColors.success,
      accent: GastroColors.success,
      softBackground: hexToRgba(GastroColors.success, 0.16),
    };
  }
  if (rating >= 3.5) {
    return {
