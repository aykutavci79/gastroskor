export type OnlineOrderCategory = {
  slug: string;
  label: string;
  hint?: string;
};

export const ONLINE_ORDER_CATEGORIES: OnlineOrderCategory[] = [
  {
    slug: 'tatli-tuzlu',
    label: 'Tatlı & Tuzlu',
...
export function categoryLabel(slug: string): string {
  ...
  return ONLINE_ORDER_CATEGORIES.find((row) => row.slug === normalized)?.label ?? slug;
}
