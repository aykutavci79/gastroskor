...
import { listDmInbox, listFriendRequests, listUserNotifications } from '@/lib/api';
...
export function AppBadgesProvider({ children }: { children: React.ReactNode }) {
  ...
  const refresh = useCallback(async () => {
    ...
    const [dm, notif, requests] = await Promise.all([
  ...
  const value = useMemo(
    () => ({ dmUnread, notificationUnread, takipPending, refresh }),
    [dmUnread, notificationUnread, takipPending, refresh],
  );
